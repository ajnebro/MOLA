...
- [`benchmarks/`](benchmarks/) — **worked benchmark-suite use cases.** Scripts that apply MOLA to
  ...
  than the CLI), and `characterize_dtlz_benchmark.py` (the synthetic DTLZ1-DTLZ7 suite, 3
  objectives by default — these *are* in the flat namespace, so a single one can also be run via
  `mola run DTLZ2`). Generated output: `re_features.csv`, `rwa_features.csv`, `dtlz_features.csv`.
